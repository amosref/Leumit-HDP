Alias: $DAR = http://hl7.org/fhir/StructureDefinition/data-absent-reason
Alias: $InfoApproved = http://fhir.leumit.co.il/StructureDefinition/ExtInfoApproved
Alias: $PopulationType = http://fhir.leumit.co.il/StructureDefinition/ExtPopulationType
Alias: $FamilyHead = http://fhir.leumit.co.il/StructureDefinition/ExtFamilyHead
Alias: $FictPatient = http://fhir.leumit.co.il/Identifier/fict-patient
Alias: $UserId = http://fhir.leumit.co.il/Identifier/UserId
Alias: $PracId = http://fhir.leumit.co.il/Identifier/PracId
Alias: $identifierType = http://terminology.hl7.org/CodeSystem/v2-0203
Alias: $tamar-visit-types = http://fhir.leumit.co.il/CodeSystem/tamar-visit-types
Alias: $or-visit-types = http://fhir.leumit.co.il/CodeSystem/or-visit-types
Alias: $expertise-code = http://fhir.leumit.co.il/CodeSystem/expertise-code
Alias: $patient-visit-reason-leumit = http://fhir.leumit.co.il/CodeSystem/patient-visit-reason-leumit
Alias: $visit-expertise-vs = http://fhir.leumit.co.il/ValueSet/visit-expertise
Alias: $sct = http://snomed.info/sct

///
Alias: $info-buckets = http://fhir.health.gov.il/cs/il-hdp-information-buckets
Alias: $LeumitBodySite = http://fhir.leumit.co.il/cs/body-site
Alias: $LeumitSpecimenType = http://fhir.leumit.co.il/cs/specimen-type
Alias: $leumit-units-of-measure = http://fhir.leumit.co.il/cs/unit-of-measure


Alias: $vs-snomed-ct = http://fhir.leumit.co.il/ValueSet/all-snomed-codes
Alias: $vs-il-core-doctor-to-doctor =   http://fhir.health.gov.il/ValueSet/il-core-doctor-to-doctor
Alias: $vs-il-core-virtual-type =       http://fhir.health.gov.il/ValueSet/il-core-virtual-type
Alias: $vs-encounter-type =             http://fhir.leumit.co.il/ValueSet/encounter-type
Alias: $vs-tamar-visit-types =          http://fhir.leumit.co.il/ValueSet/tamar-visit-types
Alias: $vs-patient-visit-reason-leumit = http://fhir.leumit.co.il/ValueSet/patient-visit-reason-leumit
Alias: $vs-leumit-units-of-measure =         http://fhir.leumit.co.il/vs/unit-of-measure
Alias: $vs-leumit-body-site =              http://fhir.leumit.co.il/vs/body-site
Alias: $vs-leumit-specimen-type =          http://fhir.leumit.co.il/vs/specimen-type

/// TEMP - Encounter
Alias: $modeOfArrival =                 http://hl7.org/fhir/StructureDefinition/encounter-modeOfArrival
Alias: $ext-encounter-paying-entity =   http://fhir.health.gov.il/StructureDefinition/ext-encounter-paying-entity
Alias: $ext-movement-number =           http://fhir.health.gov.il/StructureDefinition/ext-movement-number
Alias: $ext-encounter-visitor-type =    http://fhir.health.gov.il/StructureDefinition/ext-encounter-visitor-type
Alias: $vs-patient-mode-of-arrival-moh = http://fhir.health.gov.il/ValueSet/patient-mode-of-arrival-moh
Alias: $vs-il-core-encounter-class =    http://fhir.health.gov.il/ValueSet/il-core-encounter-class
Alias: $vs-il-core-service-type =       http://fhir.health.gov.il/ValueSet/il-core-service-type
Alias: $vs-patient-visit-reason-moh =   http://fhir.health.gov.il/ValueSet/patient-visit-reason-moh
Alias: $vs-patient-admit-source-moh =   http://fhir.health.gov.il/ValueSet/patient-admit-source-moh
Alias: $vs-patient-release-type-moh =   http://fhir.health.gov.il/ValueSet/patient-release-type-moh
Alias: $il-core-encounter-class = http://fhir.health.gov.il/cs/il-core-encounter-class
Alias: $v3-ParticipationType = http://terminology.hl7.org/CodeSystem/v3-ParticipationType
Alias: $patient-visit-reason-moh = http://fhir.health.gov.il/cs/patient-visit-reason-moh


// internal empty CSs and corresponding VSs
Alias: $leumit-lab-test-code = http://fhir.leumit.co.il/cs/lab-test-code
Alias: $leumit-specimen-group = http://fhir.leumit.co.il/cs/specimen-group
Alias: $leumit-container-type = http://fhir.leumit.co.il/cs/container-type
Alias: $leumit-lab-test-instructions = http://fhir.leumit.co.il/cs/lab-test-instructions
Alias: $leumit-confidentiality = http://fhir.leumit.co.il/cs/confidentiality
Alias: $leumit-lab-test-referral-status = http://fhir.leumit.co.il/cs/lab-test-referral-status-reason
Alias: $leumit-lab-test-order-status = http://fhir.leumit.co.il/cs/lab-test-order-status

Alias: $vs-leumit-lab-test-code = http://fhir.leumit.co.il/ValueSet/lab-test-code
Alias: $vs-leumit-specimen-group = http://fhir.leumit.co.il/ValueSet/specimen-group
Alias: $vs-leumit-container-type = http://fhir.leumit.co.il/ValueSet/container-type
Alias: $vs-leumit-lab-test-instructions = http://fhir.leumit.co.il/ValueSet/lab-test-instructions
Alias: $vs-leumit-confidentiality = http://fhir.leumit.co.il/ValueSet/confidentiality
Alias: $vs-leumit-lab-test-referral-status = http://fhir.leumit.co.il/ValueSet/lab-test-referral-status-reason
Alias: $vs-leumit-lab-test-order-status = http://fhir.leumit.co.il/ValueSet/lab-test-order-status